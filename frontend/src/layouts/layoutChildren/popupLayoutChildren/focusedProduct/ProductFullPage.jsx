    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                ...
                let direct = null;
                try { direct = await getProduct(productId); } catch { /* ignore */ }
                if (direct && !ignore) {
                ...
                const match = normalized.find(p => String(p.id) === String(productId));
    ...
    const handleAddToCart = useCallback(async () => {
        ...
        try {
            ...
            try {
                ...
                if (resp?.id) {
                    dispatch(attachBackendId({ productId: product.id, backendItemId: resp.id }));
                }
            } catch { }
            try { await logCartAdd(product); } catch { }
        } catch (err) {
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {loading && (
                        <div className="d-flex align-items-center gap-2" style={{ fontSize: '.75rem' }}>
                            <Spinner size="sm" animation="border" /> Loading product…
                        </div>
                    )}
                    {error && !loading && (
                        <div
                        ...
                        >
                            <div className="fw-semibold" style={{ fontSize: '.85rem' }}>Not Found</div>
                    ...
                    {!loading && product && (
                        <FocusedProduct
                            ...
                            onMoreLikeThis={() => navigate(`/products?category=${encodeURIComponent(product._raw?.category_id || '')}`)}
